...
int colorDetect(Mat src, Point2f target){
  ...
  if(colour.val[0] >= 160 && colour.val[0] <= 255 
    ...
    return 2;
        ...
        && colour.val[1] >= 0 && colour.val[1] <= 50
...
int main( int argc, char** argv )
{
  ...
  file.open("/dev/tty.wchusbserial1420");
  ...
  file.seekg(-1,ios_base::end); 
  	...
  	while(keeplooking){
  		...
  		if((int)file.tellg()<=1){
  			file.seekg(0);
  			keeplooking = false;//firstline==last line
		  }
		else if (ch == '\n'){
			keeplooking = false;//new line character
		}
		else{
  ...
  //If the camera if empty, stop program.
  if ( !cap.isOpened() )
    {
